...
const QcPage = () => {
  ...
  const requestfile = async () => {
    ...
    const request_file_response = await MetaRequestFile({
      count,
  ...
  return (
    <>
      ...
      <div className="w-full md:mx-auto md:w-4/6 mt-2 grid grid-cols-8 place-items-center  gap-2">
        {files.map((file, index: number) => (
          <Link
            href={`/meta/file/${encryptURLData(file.id.toString())}`}
            key={index}
          >
